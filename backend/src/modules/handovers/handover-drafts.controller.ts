...
import type { UpdateDraftItemDto } from "./dto/update-draft-item.dto";
 ...
 *   PATCH  /api/v1/handover-drafts/:id/items/:itemId — zahtev 027/26: izmena količine/napomene stavke
 *                                               (samo dok je nacrt radni — nije zaključan ni „Predat")
 *   DELETE /api/v1/handover-drafts/:id/items/:itemId — zahtev 027/26: brisanje pogrešno ubačene stavke
 *                                               (hard delete — `handover_draft_items` nema `deleted_at`; ista kapija)
 ...
 * PDM-a", Nenad 16.07), `PATCH`/`DELETE :id/items/:itemId` (zahtev 027/26,
 * Igor 26.07 — pun rad nad nacrtom kao nad dokumentom) i `decision` ruta.
...
export class HandoverDraftsController {
  ...
  /**
   * Zahtev 027/26 (Igor 26.07): izmena postojeće stavke nacrta — broj komada
   * (`quantityToProduce`) i/ili napomena. Crtež se ne menja (za zamenu: obriši
   * stavku pa je dodaj ponovo, da prođe §6.5.3/§6.5.4 provere). 404 za
   * nepostojeći nacrt/stavku, 422 za zaključan (predat) nacrt.
   */
  ...
  updateItem(
    ...
    return this.drafts.updateItem(id, itemId, dto);
  }

  /**
   * Zahtev 027/26 (Igor 26.07): brisanje pogrešno ubačene stavke iz nacrta.
   * Hard delete (tabela nema `deleted_at`) — trag ostaje u `audit_log`. Za
   * „ne ide u ovu primopredaju, ali ostaje evidentirana" postoji odluka
   * „Isključi" (§6.5.4), ovo je za stavku koja tu nije ni trebalo da bude.
   */
  ...
  removeItem(
    ...
    return this.drafts.removeItem(id, itemId);
  }

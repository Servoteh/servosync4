...
**ServoSync** is the product name for the BigBit ↔ BigTehn (QMegaTeh) sync and related tooling. This folder is the **NestJS + Prisma + PostgreSQL** backend (monorepo root is one level up). HTTP routes are served under the `/api` prefix.
...
From this directory (`backend/` — after `cd backend` from the repository root):

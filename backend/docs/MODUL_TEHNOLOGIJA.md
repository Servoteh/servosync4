...
| CNC programeri | TP sa fokusom na CNC programe + namenski CAM ekran „CAM programiranje" (lista pozicija za CAM + ček „urađeno", §2.4) |
...
`src/app.module.ts` registruje 13 domenskih modula + infrastrukturne module i jedan globalni
interceptor:
...
| `cnc-programs` | `/api/v1/cnc-programs` | CAM programiranje: lista pozicija za CAM (nezavršeni RN sa `usesPriority` operacijom) + ček „CAM urađen" sa auditom (§2.4, ODLUKE #35). |
...
`handovers`, `nacrti`, `pdm`, `structures`, `part-locations`, `mrp`, `customers`, `projects`,
`kiosk`, `reversi`, `operations-queue`, `cnc-programs`, `production-log`, `session-analytics`,
`syncs`, `completed-orders`.
...
| `tehnolog` | R W A rw | R W A L | R | R W | R W A | R | R | R | — |
...
| `menadzment` | R A | R W | R | R W | R W A | R W | R | R | R |
...
- `tehnolog` i `menadzment` **imaju** `primopredaje.approve` (ODLUKE #33, 12.07.2026) — za
  `menadzment` je to **PRIVREMENO** dok se ne razreši finalni potpisnik. Istom odlukom su
  frontend rute razdvojene: **`/nacrti`** (gate `primopredaje.write`, projektanti) vs
  **`/handovers`** (gate `primopredaje.approve`, tehnolozi).
...
Frontend je od 12.07.2026 (ODLUKE #33) **razdvojen na dve rute**: **`/nacrti`** (gate
`primopredaje.write`) — ekran projektanata za kreiranje/uređivanje/predaju nacrta — i
**`/handovers`** (gate `primopredaje.approve`) — ekran tehnologa sa četiri taba: **Na čekanju**,
**Na pisanju** (Paket A: primopredaje čiji se TP piše, brojači po tehnologu/predmetu iz
`GET /handovers/writing-stats`), **Odobrene**, **Sve primopredaje** — svaki tab mapira na jedan
pod-resurs gore.
...
> **Bug-fix `b064a96` (E2E proba na produ, 13.07.2026):** native `drawing_handovers.id` kolidirao
> je sa legacy `work_orders.drawing_handover_id` — ta legacy kolona nosi **ID NACRTA iz `tRN`**
> (opseg 1..3446, 3349 RN-ova), pa je launch native primopredaje padao 409 na tuđem RN-u.
> `submit()` sekvenca sada radi floor `GREATEST(MAX(id), MAX(legacy ref), 9999)` → native
> primopredaje žive od **10000+** do cutover remapa.
...
| `handoverWorkerId` | Int (`handover_worker_id`) | projektant koji je predao — UI labela **„Predao (projektant)"** (do 13.07. „Predato tehnologu"); NOT NULL, bez FK — u `submit()` = `designerId` nacrta (fallback 0), `handover-drafts.service.ts:901` |
...
| `isUrgent` (`is_urgent`) | Boolean | **HITNO** flag (Paket A, migracija `20260712180000`) — postavlja ga `approve()` (checkbox), briše return-to-pending; badge u listama/detalju/TP kartici + crveni „HITNO" u zaglavlju RN štampe. Nema legacy izvora. |
...
| POST | `/v1/handover-drafts` | `primopredaje.write` | kreiranje (zaglavlje + stavke); `designerId` **OPCION** (default = JWT worker; radnik mora biti AKTIVAN — hard 422); **AUTO-BOM** (proba r1): izbor glavnog sklopa automatski izlistava sve pozicije iz sastavnice (vidi (i) niže); **preduslovi stavke**: ne-odobren PDM → 422 (hard), PDF/revizija → `meta.warnings` (soft); pre-check duplikata → `pre_check_*` + warning. 422 ako projektant/predmet/crtež ne postoje |
...
| GET | `/v1/handovers/engineers` | read | aktivni radnici vrste „Inženjeri" (`engineerWorkerWhere` helper u `common/workers/technologist-criteria.ts`) — projektant ComboBox u nacrtima (proba r1) |
| GET | `/v1/handovers/writing-stats` | read | brojači „Na pisanju" po tehnologu/predmetu — puni tab „Na pisanju" (Paket A) |
...
| GET | `/v1/handovers/:id/print-bundle` (+`/pdf`) | read | P3: crtež te primopredaje za štampu; od Paketa A i dugme **„PDF crteža"** u detalju primopredaje |
| POST | `/v1/handovers/:id/approve` | `primopredaje.approve` | odobri (0→1) + dodeli tehnologa (**OBAVEZAN**) + opcioni rok + opcioni **`isUrgent`** (HITNO checkbox, Paket A); 422 ako tehnolog fali/nije aktivan tehnolog/rok nevalidan/komentar >250; 409 ako nije U OBRADI |
| POST | `/v1/handovers/:id/reject` | approve | odbij (0→2); `reason` **OBAVEZAN** (422 bez njega ili >250) |
| POST | `/v1/handovers/approve-batch` | approve | **grupno odobravanje CELE primopredaje** (ODLUKE #36): `{handoverIds[], technologistId, dueDate?, isUrgent?, comment?}` — sve pozicije istog broja nacrta (FE grupiše po `draftContext.draftNumber` jer `drawing_handovers` NEMA draft FK); best-effort `updateMany`, vraća `{approved, skipped:[{id,reason}]}`; legacy paritet `spPromeniStatusPrimopredaje` (0/1/2 grupno). **LANSIRANJE OSTAJE POJEDINAČNO.** |
| POST | `/v1/handovers/reject-batch` | approve | grupno odbijanje: `{handoverIds[], reason}` — isti best-effort obrazac kao approve-batch |
...
Od Paketa A (12.07.2026) approve prima i opcioni **`isUrgent`** (checkbox „HITNO" u dijalogu) →
`drawing_handovers.is_urgent`; return-to-pending ga briše. HITNO se prikazuje kao badge u
listama/detalju/TP kartici i kao **crveni „HITNO"** u zaglavlju štampanog RN-a. Grupna varijanta
odobravanja/odbijanja je `approve-batch`/`reject-batch` (ODLUKE #36, tabela ruta gore) — grupno
ide CELA primopredaja (sve pozicije istog broja nacrta), a **lansiranje ostaje pojedinačno**.
...
### (i) AUTO-BOM — izbor glavnog sklopa izlistava pozicije (proba r1, 13.07.2026)

Kod `POST /handover-drafts`: izbor glavnog sklopa **AUTOMATSKI izlistava sve pozicije iz
sastavnice** (`useBom` flat, rekurzivno kroz podsklopove). Pravila:

- **nabavni delovi** (`is_procurement`) se **tiho preskaču** (legacy paritet
  `WHERE ISNULL(Nabavka,0)=0`);
- **neodobreni u PDM-u** se preskaču **uz upozorenje** (isti kriterijum kao preduslovi stavke §(f));
- **sam sklop** ulazi kao prva stavka (`isMain=true`).

Uz to je `designerId` postao **OPCION** (default = radnik iz JWT-a; izabrani radnik mora biti
**AKTIVAN** — hard 422), FE nudi projektant ComboBox sa prefill-om ulogovanog
(`GET /handovers/engineers`), a labela „Predato tehnologu" je preimenovana u
**„Predao (projektant)"** (`handoverWorker` = `designerId` nacrta).
...
| `qualityTypeId` | Int (default 0) | 0=dobar / 1=dorada / 2=škart (FK `part_quality_types`). Polje „Kvalitet" na RN detalju: badge **„Dorada"** (warn) / **„Škart"** (danger) samo za 1/2, tekst „Redovan" za 0 (proba r2). |
...
| `parentWorkOrderId` | Int? (`parent_work_order_id`) | Poreklo dorada/škart child RN-a (Paket B, migracija `20260712200000`) — upisuje ga `rework()`; enrich vraća `parentWorkOrder` + `reworkChildren`; lista podržava filter `?reworkOnly=true`. |
...
| Metoda | Ruta | Permisija | Šta radi |
|---|---|---|---|
| GET | `/` | (samo JWT) | Lista + filteri (§1.4), uklj. `?reworkOnly=true` i `locations[]` po redu (Paket B). |
...
`completed` ('true'/'false' → `status`), `reworkOnly` ('true' → samo dorada/škart
child nalozi, `parent_work_order_id IS NOT NULL`; Paket B). Relacije (radnik,
kvalitet, status) se razrešavaju **batch-resolverima**, ne Prisma
required-JOIN-om — legacy orphan FK bi inače oborio 500 (memorija „legacy-read:
batch-resolve").

Od Paketa B lista vraća i **`locations[{positionCode, quantity}]`** po redu —
neto stanje iz `part_locations` (SUM po poziciji); ista informacija je kolona
**„Lokacija"** na `/completed-orders`.
...
- **rework** (Dorada/Škart, `work-orders.service.ts:1043`): child RN u istom
  ...
  = zadata količina, status = U OBRADI. Od Paketa B `rework()` upisuje i
  **poreklo** (`parent_work_order_id` = izvorni RN); enrich detalja vraća
  `parentWorkOrder` (na childu) i `reworkChildren` (na izvoru), a lista ima
  filter `?reworkOnly=true`.
...
preko `pdfmake`: zaglavlje (logo Servoteh + `RNZ` barkod; kad je primopredaja
HITNA — **crveni „HITNO"** u zaglavlju, Paket A), info tabela
(komitent/predmet/crtež/materijal/**dimenzija materijala** (`materialDimension`,
proba r2)/rok/tehnolog/revizija) i tabela operacija
...
> Napomena: ovo je **prioritet operacija** (planska tabla). Namenska CAM lista
> pozicija + ček „CAM urađen" je zaseban modul `cnc-programs` (§2.4).

#### 2.4 CAM programiranje — modul `cnc-programs` (Paket B, ODLUKE #35)

NOVI modul `src/modules/cnc-programs` + tabela **`cnc_programs`** (migracija
`20260712210000`) — realizuje ODLUKE #8 (`cnc_programs` tabela). Nav stavka
**„CAM programiranje"**. Gate: read = `tehnologija.read`, write =
`tehnologija.write`.

| Metoda | Ruta | Permisija | Šta radi |
|---|---|---|---|
| GET | `/v1/cnc-programs` | `tehnologija.read` | Lista pozicija za CAM = nezavršeni RN sa operacijom na `usesPriority` RC-u (17.0/17.1 CAM Programiranje). |
| PATCH | `/v1/cnc-programs/:workOrderId` | `tehnologija.write` | `{isDone, note?}` — ček „CAM urađen" sa auditom `completedByWorkerId`/`completedAt` iz JWT-a. |

**CAM filter (proba r2):** lista filtrira (izbacuje) pozicije čija je trojka već
otkucala **CNC glodanje/struganje** (RC naziv počinje „CNC" — CAM prethodi tim
operacijama, pa je implicitno urađen) ILI **završnu kontrolu**
(`significantForFinishing`) → lista pala sa **549 na 271 poziciju**. Univerzalno
(ručno) glodanje/struganje je IZUZETO iz filtera.
...
| GET | `/worker/open?card=` | `tehnologija.report_work` | Kiosk „Moji otvoreni" (ODLUKE #36): otvoreni postupci radnika (kartica ili JWT worker) + `hasOpenSession`; zatvaranje iz liste kroz postojeći `POST /:id/finish` BEZ ponovnog skeniranja. |
...
- **Kucanja** — paginirana lista `tech_processes` (`list()`), expand otvara
  Karticu TP. Od Paketa A lista razdvaja kolonu **„Radnik"** (ko je kucao) od
  nove kolone **„Tehnolog"** (autor TP-a — `work_orders.worker_id`,
  batch-resolve preko trojke).
...
**„Moji otvoreni"** (ODLUKE #36): panel sa listom otvorenih postupaka
prijavljenog radnika (`GET /tech-processes/worker/open?card=` — kartica ili JWT
worker, + `hasOpenSession`); zatvaranje operacije direktno iz liste kroz
postojeći `POST /:id/finish`, **bez ponovnog skeniranja** RN + operacija barkoda.
...
10–11.07, probe na produkciji + paketi dorada A/B 12–13.07.
...
| **Probe na produkciji + dorade** (12–13.07, ODLUKE #33–#36) | Paketi A (#34) i B (#35), probe runda 1+2, `b064a96` id-floor fix | Razdvajanje `/nacrti` vs `/handovers` + `primopredaje.approve` za tehnolog/menadzment (#33); HITNO flag + tab „Na pisanju" + kolone Radnik/Tehnolog u Realizaciji (Paket A); `parent_work_order_id` + `locations[]` + NOVI modul `cnc-programs` (Paket B); AUTO-BOM + `designerId` opcion + „Predao (projektant)" (proba r1); approve-batch/reject-batch + kiosk „Moji otvoreni" + CAM filter 549→271 (proba r2, #36); E2E tok na produ + id-floor fix `b064a96`; login parnost 1.0→2.0 + 17 biro naloga |

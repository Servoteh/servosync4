...
| 37 | Read-only test nalog | `test@servoteh.com` (users id 49, rola `menadzment` — VIDI sve) je **read-only**: env `AUTHZ_READONLY_USER_IDS=49` + globalni `ReadOnlyInterceptor` (posle per-controller JwtAuthGuard) → svaka mutacija (POST/PUT/PATCH/DELETE, izuzetak čist-read `barcode/decode`) pada 403 „Test nalog je samo za pregled — izmene nisu dozvoljene."; login/sso/me vraćaju `readOnly:true` pa front prikazuje trajni baner. Paritet u 1.0: frontend gate u `services/supabase.js` (`TEST_READONLY_EMAILS`) + toast + baner (tamo je blok aplikativni, RLS i dalje pušta menadzment). Rollback = obriši env red + `docker compose up -d` | Nenad 2026-07-14 |
| 38 | Novi PREDMETI u prelaznom periodu (presuda N3) | **Dual unos sa paritetom brojeva**: predmet se otvara PRVO u 3.0 (`projects-write`, role sa `projects.write` — komercijala/menadžment/admin; 3.0 dodeljuje broj od 10001+), pa se ISTI broj ručno prekuca u BigBit (nosilac fakturisanja do F5). Sync guard (`ADDITIVE_DEDUP_FIELDS`, `generic.syncer`): BigBit kopija istog broja se preskače uz upozorenje — 3.0-native red je istina u 3.0; ranije ušla kopija se self-heal-uje. **Precizira #23 za predmete**; komitenti i dalje SAMO u BigBit-u (u 3.0 stižu ručnim sync-om, `/syncs` stranica). Detalji: [../../docs/ODLUKA_N3_predmeti_sync.md](../../docs/ODLUKA_N3_predmeti_sync.md) | Nenad 2026-07-22 |
| 39 | Scheduler pogon (Talas A) | **In-process tik u NestJS-u bez novih zavisnosti** (obrazac grid-autofill: setInterval 30s + unref), umesto pg_cron na glavnoj bazi (traži shared_preload_libraries + restart PROD Postgres-a) ili @nestjs/schedule (§10). Garancije: `scheduled_job_runs` UNIQUE(job_key,scheduled_for) = atomski claim (tačno-jednom i sa više instanci), catch-up prozor 180min (deploy preko termina ne guta posao), retry ≤3. Rasporedi LOKALNO (Europe/Belgrade, sopstveni DST util) — 1.0 dupli UTC slotovi nepotrebni. Poslovi = tanki pozivi POSTOJEĆIH sy15 SECURITY DEFINER fn (12 kom; ŽIVA provera 24.07: sve enqueue automatike RADE na sy15 pg_cron — Talas A je SEOBA POGONA za F5, ne oživljavanje; paritet-audit nalaz „tiho mrtve" ispravljen). Dispatch (slanje) OSTAJE sy15 VM cron+edge do dekomisije (→ Talas F registar). Aktivacija: `SCHEDULER_ENABLED=true` u backend.env; cutover = uključi → uporedi dan-dva → `cron.unschedule` na sy15 (reverzibilno). | Nenad 2026-07-24 („talas a idemo") |

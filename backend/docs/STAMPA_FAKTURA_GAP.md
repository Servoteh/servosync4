...
| 5 | **Datum prometa dobara / Datum prometa / Date of delivery** | IFR, IFGP, IFUSL, ino usluga | NE | `Invoice` ima samo `documentDate` (`:3852`) i `dueDate` (`:3853`). SEF-u isto fali (`sef-incoming.service.ts:456` beleži da se `cbc:ActualDeliveryDate` ne parsira) | **`Invoice.supplyDate DateTime?`** (kolona `supply_date`) — koristi i štampa i UBL `cbc:ActualDeliveryDate`. ⚠️ Predlog je 01.08. glasio `deliveryDate`; pri spajanju sa `main` 02.08.2026. je uzeta postojeća kolona `supply_date` (isti podatak, već vezan na SEF), a dvojnik uklonjen |

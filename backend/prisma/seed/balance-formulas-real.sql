-- ============================================================================
-- balance-formulas-real.sql — balance_formula_definitions (BigBit ZR motor)
-- ============================================================================
-- IZVOR / STATUS (vidi backend/docs/migration/44-zr-bilans-motor-iz-vba.md):
--   Motor BigBit ZR je DATA-DRIVEN: prave formule su u Access tabeli
--   ZR_AOP_Modla (kolona Definicija), a NE u VBA kodu. Ta tabela NIJE izvezena
--   u CSV (u .MDB je binarna; postoji samo uputstvo _legacy/BigBit26/_EXPORT_ZR.txt).
--   => Ovaj seed je DELIMIČAN / REKONSTRUKCIJA:
--      - DSL sintaksa i semantika: VERBATIM iz modula ZR (BigBit_APL_2010_ZR_code.txt).
--      - Pojedinačne AOP formule: REKONSTRUISANE iz standardnog APR obrasca +
--        kontnog plana Servoteha (klase 0–7). NISU verbatim ZR_AOP_Modla.
--   ⚠️ PRE PRODUKCIJE: zameniti pravim ZR_AOP_Modla dumpom (vidi doc 44 §7).
--
--   DSL (verbatim iz koda, doc 44 §2):
--     D<maska>*  / P<maska>*   = Σ Duguje / Potrazuje  (LIKE nad [Konto])
--     PSD<maska>*/ PSP<maska>* = početno stanje dug. / potr.
--     A<aop>                   = Iznos_1 druge AOP pozicije (TG); Iznos_3 (PG)
--     AB<aop> / AC<aop>        = Iznos_2 / Iznos_3 druge pozicije  (NIJE u našem GkEval-u!)
--     + - ( )                  = operatori;  * iza konta = wildcard (NE množenje)
--
--   RAZLIKE PREMA NAŠEM GkEval (doc 44 §8) — seed ih izbegava gde može:
--     - A-reference u BigBit-u se KLAMPUJU na >=0 (IIf(...>0,...,0)). GkEval ne
--       klampuje. Ovde su A-formule pisane tako da su prirodno >=0; ako neka može
--       biti negativna, biće potreban clamp flag (doc 44 §8 t.2).
--     - AB/AC nisu korišćeni u ovom seed-u (GkEval ih još ne parsira).
--
--   statement_type: BALANCE_SHEET | INCOME_STATEMENT  (SI/statistički van ovog seed-a)
--   UNIQUE (statement_type, aop).
-- ============================================================================
BEGIN;
TRUNCATE TABLE balance_formula_definitions RESTART IDENTITY CASCADE;

-- ── BILANS STANJA (BALANCE_SHEET) ───────────────────────────────────────────
-- Neto pozicije = bruto - ispravka (npr. 022 - 0229). Ključna mapiranja iz doc 37 §C/§D.
...
-- source: REKONSTRUKCIJA (doc 44 §5/§8) — DSL verbatim iz modula ZR; AOP formule
--         iz standardnog APR obrasca + kontni plan Servoteha. Prave ZR_AOP_Modla
--         formule su binarne u .MDB → izvezi (doc 44 §7) i zameni pre produkcije.
-- BALANCE_SHEET: 30 redova ; INCOME_STATEMENT: 25 redova.
-- STATUS: DELIMIČAN. Fali: (1) verbatim Definicija po AOP-u iz ZR_AOP_Modla;
--   (2) kompletan skup AOP-a po veličini firme (mikro/malo/srednje);
--   (3) SI (statistički izveštaj) obrazac; (4) kontrolna pravila (ZR_AOP_Pravila);
--   (5) AB/AC kolone i clamp>=0 (traže GkEval dopune, doc 44 §8).

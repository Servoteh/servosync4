...
- **REŠENJE (25.07, migracija `20260725230000_katbroj_brana`) — tri sloja:**
  1. **DB ratchet-trigger** `guard_catalog_unique` na `items`: odbija svaki NOV duplikat i svaku promenu šifre u već zauzetu vrednost (case-insensitive, trimovano), a TOLERIŠE zatečene duplikate da sync i ažuriranja ne pucaju. Uz funkcionalni indeks `idx_items_catalog_lower` (bez njega bi provera bila seq scan nad 92k redova pri svakom upisu).
  2. **Sync guard** (`SOURCE_UNIQUE_FIELDS` u `table-ownership.ts` + `generic.syncer.ts`): duplikat iz BigBit-a se odbacuje PRE upisa (prvi red po ključu pobeđuje) i beleži u sync log. Nužno jer `items` ide FULL REFRESH pod `session_replication_role='replica'` — tamo su trigeri isključeni, a tvrd UNIQUE bi oborio ceo `createMany` chunk → šifarnik artikala bi ostao prazan.
  3. **Tvrd UNIQUE indeks** (`uq_items_catalog_lower`, parcijalni + `lower()`) — komanda pripremljena na dnu migracije; pušta se čim provera duplikata vrati 0, uz `DROP TRIGGER`.
- Verifikovano na dev bazi: nov duplikat ODBIJEN · nova jedinstvena šifra PROLAZI · case-varijanta postojeće šifre ODBIJENA · ažuriranje zatečenog duplikata PROLAZI · promena šifre u zauzetu ODBIJENA. Sync guard pokriven jedinicnim testovima.
- Preostalo (poslovni deo, Nenad): prenos jedinstvenih brojeva u BigBit po radnom dokumentu `KATBROJ_ciscenje_radni_dokument_2026-07-25.csv` (1.170 redova „A — isti naziv, spoji" / 3.128 „B — presudi"); watchdog u monitoringu prati da broj ne raste.

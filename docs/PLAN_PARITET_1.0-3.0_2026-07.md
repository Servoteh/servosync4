...
>   (loc_*/scada/po_cleanup infra poslovi OSTAJU na sy15 do seobe svojih podsistema). **→ Verifikovano 06.08:** izvedeno 25.07 — svih 15 sy15 cron-ova ugašeno, 3.0 je od tada jedini pogon (bez fallback-a).

| # | Stavka | Nalazi |
|---|--------|--------|
| ✅ A1 | **Scheduler pogon** u NestJS backend-u. Odluka: `@nestjs/schedule` u procesu (nova zavisnost → traži odobrenje po BACKEND_RULES §10) ili pg_cron na glavnoj bazi + tanki HTTP okidači. Preporuka: pg_cron (već postoji znanje sa sy15, preživljava restart deploy-a, ne duplira se pri skaliranju). → **Verifikovano 06.08:** in-process scheduler u `backend/src/modules/scheduler/` (SCHEDULER_ENABLED kapija, atomski claim ON CONFLICT(job_key, scheduled_for), run-now, `ScheduledJobRun` u schema.prisma) — commit e2813cc0; ODLUKA #39 presudila u korist in-process rešenja, ne pg_cron-a. | X-EMAIL-AUTOMATIKE-01 |
| ✅ A2 | Na pogon nakačiti poslove za koje 3.0 **već ima config/outbox**: isteci ugovora/dokumenata/LK (lead-days config postoji), korektivne mere 07:30, rođendani/godišnjice/digest (flagovi postoje), onboarding dnevni podsetnik, rokovi vozila (RPC `maint_check_vehicle_deadlines` postoji — samo cron), podsetnici za sastanke (30 min + dnevni za akcije), sedmični sastanak petak 08h (+DST guard + prenos tema — logika u 1.0 sql-u kao referenca). → **Verifikovano 06.08:** tačno 12 poslova u `scheduler/sy15-cron-jobs.ts` (kadr-hr-reminders/-corrective/-onboarding/-attendance-alerts/-attendance-digest/-weekly-risk/-qbt-cards, sast-action-reminders/-meeting-reminders/-weekly-auto, maint-deadlines, pb-enqueue). | X-EMAIL-AUTOMATIKE-02..08 |
| ✅ A3 | **Dispatch odluka:** slanje mejlova iz 3.0 (sopstveni worker nad outbox tabelama) ili svesno zadržati sy15 dispatch workere do dekomisije (Blok B RADNI_PLAN-a). Ako ostaje sy15 — upisati u registar sy15-duga (Talas F). → **Verifikovano 06.08:** prevaziđeno nabolje — dispatch je prenet u 3.0 (`notify-dispatch.service.ts` 909486dd, A-2a; `sastanci-dispatch.service.ts` f4eaa651, A-2b, iza DISPATCH_ENABLED), pa ne ostaje na sy15 kako red tvrdi. | X-EMAIL-AUTOMATIKE-09 |

## 2. Talas B — Kritične funkcionalne rupe (~2–3 MN)

| # | Stavka | Sev/Effort |
|---|--------|-----------|
| ✅ B1 | Admin reset lozinke → upis u **oba** auth sistema (3.0 `users.passwordHash` + GoTrue dok sy15 živi). → **Verifikovano 06.08:** `podesavanja-users.service.ts:154-203` resetPassword sada piše u oba sistema (GoTrue + bcrypt hash u 3.0 `users.passwordHash`), commit 5542c660. | 🟠 S |
| ✅ B2 | Self-service promena lozinke + sprovođenje `must_change_password` na 3.0 loginu. → **Verifikovano 06.08:** `auth.controller.ts` POST /auth/change-password → auth.service.ts changePassword, uz `must_change_password` enforce u auth-context.tsx/login/mob-prijava (commit 5542c660). | 🟡 M |
| 🟨 B3 | Plaćeno odsustvo: vratiti katalog osnova + godišnji fond 5 dana (sada slobodan tekst). → **Verifikovano 06.08:** **(DELIMIČNO)** katalog osnova je vraćen (moj-profil-mutation.dto.ts PAID_LEAVE_CODES + odsustva/shared.ts PAID_LEAVE_CATALOG), ali kumulativni GODIŠNJI FOND od 5 dana i dalje ne postoji — nema računanja iskorišćeno+projektovano po godini ni kape (grep 0 pogodaka van labele). | 🟠 M |
| ⬜ B4 | „Promocija akcione tačke u aktivnost" — dugme postoji, backend vraća **501**. Implementirati ili sakriti dugme. → **Provereno 06.08 — i dalje otvoreno:** `pracenje.service.ts:690-695` promoteAkcionaTacka i dalje vraća 501, a dugme NIJE sakriveno (`rn-view.tsx:858` „Iz Sastanaka" vodi u grešku); seoba sastanaka postoji samo na ne-merge-ovanoj grani. | 🟡 L |
| ✅ B5 | Provera pa sanacija 2 BLOCKER-a iz cutover audita Lokacija (mogu biti već sanirani): PL-01 pretraga → 500; PLK-01 definitions-audit → 500. → **Verifikovano 06.08:** oba blokera sanirana — PL-01 (`part-locations.service.ts` EXISTS podupiti umesto bind-overflow-a) i PLK-01 (`locations.service.ts:1255` BigInt→Number), uz regresione testove. | 🟠 S–M |
| ⬜ B6 | Praznici: admin CRUD (sada samo read) — utiče na grid sati i GO obračun za 2027. → **Provereno 06.08 — i dalje otvoreno:** i dalje samo GET /admin/holidays i GET /kadrovska/holidays — nijedan POST/PATCH/DELETE u backend/src (grep 0). | 🟡 M |

## 3. Talas C — Štampa / PDF / izvozi (~3–4 MN)

- ✅ **C1** 🟡 M — Batch TP nalepnice direktno na TSC termalni štampač (magacin svaki dan; sada samo pregledač+Ctrl+P). `LOKACIJE-02` **→ Verifikovano 06.08:** commit dee2727b — `stampa-tab.tsx` runThermalBatch() šalje TSPL2 direktno na TSC preko /locations/labels/print.
- ✅ **C2** 🟡 M — OCR broja predmeta/TP sa nalepnice (Tesseract iz slike — nije skener-kamera tema). `LOKACIJE-01` **→ Verifikovano 06.08:** isti commit — tesseract.js@5.1.1 self-hosted (public/tesseract/), `label-ocr.ts` localTesseractOptions; isOcrEngineAvailable više ne zavisi od window.Tesseract.
- ⬜ **C3** 🟡 L — RN „sa slikama" (skice operacija u PDF). `X-STAMPA-PDF-01` **(potvrđeno 06.08)** `work-order-print.service.ts` i dalje zna samo `std|bez-barkoda`; model WorkOrderOperationImage postoji ali se ne ugrađuje u PDF.
- ⬜ **C4** 🟡 S — 360° PDF: ugraditi radar grafikon. `X-STAMPA-PDF-02` **(potvrđeno 06.08)** `hr-pdf/assessment.ts` i dalje bez radar/addImage/toDataURL.
- 🟨 **C5** 🟡 M — Poster sistematizacije (A0 + varijanta sa imenima + skok na „moja pozicija"). `KADROVSKA-01` (PARTIAL) **(DELIMIČNO)** A0 poster i deep-link `#pos-<id>` i dalje NE postoje; novo od 05.08 je `sistematizacija-section.tsx` — pravni PDF/DOCX dokument (tekst), što je druga stvar. Ekvivalenti ostaju tekstualni OrgReport + pojedinačni „Opis pozicije (PDF)".
- ⬜ **C6** ⚪ S×6 — sitni izvozi: organogram XLSX/PDF (`KADROVSKA-02`), payslip JMBG+tip rada (`ZARADE-01`), CSV akcija sastanka + štampa (`SASTANCI-01`), PB CSV kolona „Kašnjenje" (`PB-PROJEKTOVANJE-02`), CMMS CSV limit 200 redova + troškovi po stavci (`ODRZAVANJE-01/-02`), Gantt PDF legenda (`MONTAZA-03`). **(potvrđeno 06.08)** svih 6 pod-stavki otvoreno (OrgReport bez izvoza, payslip bez jmbg/tipa rada u pozivu, detalj-akcije bez CSV/print/filtera, PB CSV bez „Kašnjenje", CMMS cap 200 + granularnost po RN-u, Gantt PDF bez legende tipa).
...
> **Verifikacija 06.08:** ✅ `PROFIL-GO-05` (picker „Za koga" u MakeupModal/PaidLeaveModal uz IDOR branu
> resolveSubmitTarget, commit de5390a8), ✅ `-06` (ChangeRequestModal objašnjava posledicu za ODOBREN GO,
> zahtev 026/26, commit 48fe0e57) i ✅ `-07` (vikend/≥8h klijentska validacija + badge „🏖 Dan odmora")
> su urađeni; 🟨 `-03` ima akrual „preostalo do danas" (moj-profil.ts vacationRemaining, zahtev 028/26),
> ali avans-upozorenje/marker u modalu zaposlenog i dalje fali (postoji samo na HR ekranu saldo-tab.tsx);
> ⬜ `-02`, `-04` i `RAZVOJ-360-01` ostaju otvoreni. Uz njih je ✅ `PROFIL-GO-01` (katalog osnova) — v. B3.
...
> **Verifikacija 06.08:** ✅ SCADA stale-badge (`solar-sigen.js:126-152` + applyTheme bez reload-a u HMI
> fajlovima, commit 1375f253 + f8c04f2a — ali HOST i dalje remount-uje iframe, v. ⬜ `ENERGETIKA-02`) i
> ✅ stat kartice u Korisnicima (`korisnici-tab.tsx:76-97`). 🟨 delimično: `X-EXPORT-UX-02` (normalize() dobio
> još jedno mesto — kadrovska/prisustvo/live-rows.ts — ali ~60 tabelarnih filtera i dalje bez folding-a),
> `X-EXPORT-UX-05` (window.confirm pao sa 118/182 na 80 fajlova, jedinstvene ui-kit komponente i dalje nema),
> `X-EXPORT-UX-04` (sticky zaglavlje/kolone u grid-table.tsx, bez proxy trake), `ZARADE-02` (live totali samo za
> dirty red) i Reversi master-detalj (sadržaj 100% u modalu, fali ruta + deep-link). Ostalo ⬜ otvoreno —
> `X-EXPORT-UX-01` je čak i porastao: 199 native `type="date"` u 104 fajla (jul: 152 u 81).
...
> **Verifikacija 06.08:** ✅ **TALAS-F-1** dispatch workeri migrirani u 3.0 (`notify-dispatch.service.ts` 909486dd
> + `sastanci-dispatch.service.ts` f4eaa651) · ✅ **TALAS-F-3** nativni tok za ocenjivača (AUDIT-K6, moj-profil
> assertMyRater + rater-assessment-section.tsx; legacy link samo iza praznog env-a) · ✅ **TALAS-F-4** WhatsApp
> se šalje direktno iz 3.0 preko Meta Graph v20. 🟨 **TALAS-F-2** javna no-JWT RSVP stranica NAMERNO ostaje na
> sy15 edge-u (`sastanci-dispatch.service.ts:150-170`). ⬜ **TALAS-F-5** loc-sync/SCADA watchdog i dalje sy15
> pg_cron (3.0 ima samo read-only health prikaz) · ⬜ **TALAS-F-6** formalan popis edge funkcija nikad nije
> napravljen. Preostalo je konsolidovano u **§9**.

## 7. Šta je NAMERNO van ovog plana

- ✅ **Skener/kamera + APK ljuska** — paralelna radna linija (kamera-skener engine, /mob/lokacije). **→ Verifikovano 06.08:** paralelna linija zaista postoji i napreduje nezavisno (lib/barcode-decoder.ts).
- ✅ **Reversi dubinski paritet** — postojeći plan R1–R5 (~5,2 MN; trenutni paritet 13%) važi; ovaj
  ...
  CUTOVER_AUDIT_reversi i dalje tačna (Aneks B). **→ Verifikovano 06.08:** plan R1–R5 i dalje postoji kao zaseban dokument (ali sama statistika 13% je prevaziđena — v. Aneks B).
- ⚰️ **Praćenje O8/F5b HITNE stavke** — već u toku u `wt/pracenje-f1` liniji (F3/F5 planovi). **(PREVAZIĐENO) 06.08:** rad iz `wt/pracenje-f1` je slegao na main, a O8 je apsorbovan Cutover-om 1.0 (PR #95) — legacy ekran koji je trebalo preusmeriti više ne postoji.
- ✅ **4.0 ERP (BigBit zamena)** — poseban program (MASTER_PLAN_GRADNJE, 158 gapova). **→ Verifikovano 06.08:** MASTER_PLAN_GRADNJE_4.0 je i dalje zaseban aktivan program.

## 8. Predlog redosleda izvršenja

1. ✅ **Odmah (ova nedelja):** B1+B2 (lozinke — bezbednost), A1 odluka o scheduler-u (blokira A2), B5 provera blokera. **→ Verifikovano 06.08:** B1, B2 i B5 zatvoreni, A1 odluka doneta (ODLUKA #39, in-process scheduler).
2. 🟨 **Sledeće:** Talas A ceo (automatike su „tihi" gubitak — niko ne primeti dok ne zaboli), pa B3/B4/B6. **(DELIMIČNO) 06.08:** ceo Talas A urađen (12 poslova + cutover 25.07) i B3 delimično; B4 (501) i B6 (CRUD praznika) i dalje otvoreni.
3. 🟨 **Zatim:** Talas C (štampa — magacin i kadrovska je osete svaki dan), paralelno D (male stavke, veliki utisak). **(DELIMIČNO) 06.08:** od Talasa C urađeni samo C1 i C2 (TSC štampa + OCR); od Talasa D urađeni PROFIL-GO-01/-05/-06/-07, ostatak otvoren.
4. 🟨 **Kontinuirano:** E quick-win fond (1–2 stavke uz svaki drugi posao), F uz Blok B dekomisiju. **(DELIMIČNO) 06.08:** Talas F delimično (dispatch/WhatsApp/rater urađeni; RSVP, watchdog i popis edge fn nisu), a Blok B dekomisija je i dalje otvorena — v. **§9**.
...
## 9. Šta ostaje da se sy15 potpuno ugasi (konsolidovano 06.08.2026)

Gašenje 1.0 fronta (03.–05.08, `PROXY_1_0_AKTIVAN=false`) **nije ugasilo sy15** — ta baza ostaje
temelj 3.0: koristi je 161 fajl u `backend/src` preko drugog Prisma datasource-a
([ANALIZA_GASENJE_1.0](ANALIZA_GASENJE_1.0_2026-08-03.md) §2). Seobu domena pokriva
[PLAN_GASENJA_SY15](PLAN_GASENJA_SY15_2026-08-03.md), pisan iz ugla **tabela, RLS-a i
`SECURITY DEFINER` funkcija**. Ovaj gap-audit je otkrio drugi sloj istog duga — **preostalu
infrastrukturu OKO sy15** (edge funkcije, pg_cron watchdog-e, most ka BigTehn kešu, drugi
datasource) — koji taj plan delom ne pominje. Ova sekcija spaja oba pogleda u jedan spisak.

### 9.1 Seoba domenskih tabela — sažetak tuđeg plana (ne dupliramo)

Redosled i mera posla su u [PLAN_GASENJA_SY15_2026-08-03.md](PLAN_GASENJA_SY15_2026-08-03.md)
(revizija 2, 05.08): podaci živih domena staju u **~17 MB**, ali logika je **~200 živih DEFINER
funkcija** koje se prepisuju u NestJS, a sy15 ima **tri pisca** — SCADA relej (4,09 mil. izmena),
bridge/.mdb (~262 hilj.) i samu 3.0 aplikaciju (~20 hilj., najmanji). Zato aplikacija može
potpuno preći, a baza i dalje ostaje živa.

| # | Korak (po tom planu) | Status prema tom dokumentu |
|---|---|---|
| 0 | Mapa identiteta sy15 `auth.users` ↔ 3.0 `users` (67 FK kolona) | ⬜ NIJE POČETO — preduslov svakog koraka; sastanci ga zaobilaze mejlom (23/23 ljudi ima 3.0 nalog) |
| 1 | **Sastanci** (27 tabela / 1.120 redova / 74 fn) | 🟨 U TOKU — grana `feat/sy15-seoba-sastanci-pb`: prenos dokazan 1.120/1.120 i idempotentan, prekidač `SASTANCI_PB_IZVOR`, 4 fn prepisane (+26 testova); ostaje ~61 DEFINER fn (4–6 dana) + enqueue (1–2 dana) |
| 2 | **Održavanje** (41 fn, 34 tabele + 34 trigera, 469 MB fajlova) | ⬜ NIJE POČETO — najviše mehaničkog posla, prvi ozbiljan ulazak storage seobe |
| 3 | **Reversi + Lokacije zajedno** (59 fn, 21 tabela / 5.659 redova) | 🟨 TEMELJ POSTAVLJEN 07.08 — [SEOBA_REVERSI_LOKACIJE](SEOBA_REVERSI_LOKACIJE_2026-08-07.md): 21 Prisma model, offline migracija (dokazana nad kopijom prod šeme), prenos idempotentan i dokazan (21/21 tabela + sva 4 rev↔loc šava), prekidači `REVERSI_IZVOR`+`LOKACIJE_IZVOR` sa branom sprege. Stara grana `feat/sy15-seoba-reversi` ODBAČENA (128 commit-a iza, pokrivala samo rev). **Ostaje 16,5–24,5 dana** — kritični put je prepis `rev_issue_reversal`/`rev_confirm_return`/`loc_create_movement` (5–8 dana). 🔴 Najhitniji domen: lokacije su JEDINE u koje se uživo piše u sy15 |
| 4 | **Kadrovska** (58 fn, plate/audit/poverljivost) | ⬜ NIJE POČETO — svesno „tek posle tri uvežbana kruga" |
| 4b | **Projektni biro** (32 fn; podaci već preneti u koraku 1) | ⬜ BLOKIRAN korakom 4 — `pb_current_employee_id()` traži `employees`/`departments`/`job_positions` |
| 5 | **SCADA + bridge preusmeravanje** | ⬜ NIJE POČETO — gateway kod JESTE u monorepou (`scada/`), ali baza nije preseljena: `servosync-pg` nema nijednu `scada_*` tabelu |
| 6 | „Ostalo" (`ai_chat_*`, `assessment_*`, diktafon, kiosk, GoTrue, storage) | ⬜ NIJE POČETO — ~35 fn koje nijedan domenski korak ne pokriva |

### 9.2 Infrastruktura oko sy15 koju plan seobe domena NE pokriva

Ovo je doprinos gap-audita: stavke koje ne žive u domenskim tabelama nego u edge sloju, pg_cron-u
i mostovima — a bez njih se baza ne može ugasiti bez tihe štete.

| Šta | Status (ova verifikacija) | Zašto blokira gašenje |
|---|---|---|
| **Javni RSVP magic-link** (`sastanci-rsvp` edge fn) — `X-EMAIL-AUTOMATIKE-10`, `TALAS-F-2` | 🟨 DELIMIČNO — 3.0 generiše link i ima autentifikovan RSVP, ali javnu no-JWT obradu NAMERNO radi sy15 edge (`sastanci-dispatch.service.ts:150-170`) | Svaka pozivnica poslata IZ 3.0 nosi link ka sy15 funkciji; tokeni su preneti doslovno da stari linkovi rade. Gašenje edge sloja obara potvrdu dolaska za sve nelogovane učesnike |
| **Loc-sync health + SCADA watchdog** (`loc_sync_health_check_hourly`, `scada_watchdog_every_5_min`) — `TALAS-F-5`, `X-EMAIL-AUTOMATIKE-13` | ⬜ NIJE — nema loc/scada posla u 3.0 scheduler-u; 3.0 ima samo read-only health prikaz, a poslovi su pg_cron U SAMOJ sy15 bazi | Alarm „relej/worker je pao" gasi se zajedno sa bazom. Ispad SCADA releja ili loc-sync-a posle toga niko ne bi primetio |
| **Popis svih sy15 edge funkcija** (14 kom.) — `TALAS-F-6` | ⬜ NIJE — formalan inventar nikad nije napravljen (nema ga ni u RADNI_PLAN_3.0.md ni u OTVORENI_POSLOVI.md) | Plan seobe pokriva tabele i funkcije U BAZI; edge sloj je van njega. Bez spiska se ne zna šta se tačno gasi ni ko to još zove |
| **Konsolidacija dva PG datasource-a** — `Blok-B3` | ⬜ NIJE — `backend/prisma/sy15.prisma` je i dalje zaseban datasource; `reversi.service.ts` i `auth.service.ts` čitaju sy15 direktno | Dok postoji drugi datasource, prekidači po domenu ne pokrivaju kod koji zaobilazi branjene getere (isti obrazac kao 7 scheduler poslova sa `$queryRaw` — PLAN_GASENJA §5, revizija 2 t.5) |
| **PostgREST + GoTrue dekomisija** — `Blok-B4` | 🟨 DELIMIČNO — 1.0 front ugašen 05.08, ali PostgREST/GoTrue i sy15 su živi: 161 fajl u `backend/src`, 60 naloga i 9 prijava u 7 dana kroz GoTrue | GoTrue je poslednji korak (korak 6): 67 FK kolona gađa `auth.users`, pa se ne dira dok mapa identiteta (korak 0) ne postoji |
| **Loc-most repoint** — `Blok-B1` | 🟨 DELIMIČNO — kod sletanja postoji (`loc-tp-feed.service.ts`), živa sekvenca čeka prozor; RUNBOOK nepromenjen | Preduslov za F5c i za korak 3 (reversi+lokacije); dok se ne izvrši, lokacije zavise od BigTehn keša u sy15 |
| **F5c — Lokacije native ingest** | ⬜ NIJE — `locations.service.ts` i dalje SAMO čita `loc_location_movements` iz sy15, bez nativnog writer-a | Dok lokacije nemaju nativan upis, cron `loc_bigtehn_ingest_5min` (na 5 min, U BAZI) mora da živi — a on je jedan od 5 aktivnih sy15 poslova |
| **F5d — fizičko gašenje mosta** | ⬜ NIJE — LocTpFeedService i dalje registrovan, nema DROP migracija za `bigtehn_*` keš | Zavisi od F5c. Bez njega u sy15 ostaju 3 keš tabele + view lanac + pg_cron posao, tj. baza se ne može ugasiti ni kad domeni pređu |
| **Praćenje: promocija akcione tačke (501)** — `PROIZVODNJA-PRACENJE-01`, `B4` | ⬜ NIJE — `pracenje.service.ts:690-694` i dalje baca 501; akcioni plan čita sy15 uuid ključeve | Zavisi od koraka 1 (sastanci). Ako se sastanci presele a ovo se ne prevede, 501 ostaje trajno i dugme u `rn-view.tsx:858` i dalje vodi korisnika u grešku |
| **Energetika theme-sync** — `ENERGETIKA-02` | ⬜ NIJE — `hmi-host.tsx` remount-uje iframe; postMessage `scada-theme` se ne šalje iako ga shim sluša | NE blokira gašenje, ali dira isti fajl kao prevođenje `energetika.service` na 3.0 Prisma (korak 5) — jeftino je uraditi u istom prolazu |

### 9.3 Konačan checklist — redosled po zavisnostima

Spojeno iz oba izvora; `[x]` = urađeno i verifikovano 06.08.

**Već zatvoreno (temelj):**

- [x] **Talas A** — scheduler pogon u 3.0 (`backend/src/modules/scheduler/`, 12 poslova, cutover 25.07); bez njega nijedna automatika nije mogla sa sy15
- [x] **TALAS-F-1 / -F-4** — dispatch mejl-outboxa (hr/maint/pb/sastanci) i WhatsApp kanal rade iz 3.0
- [x] **TALAS-F-3** — nativni tok za ocenjivača (`ocena.html` više nije uslov)
- [x] **Cutover 1.0 fronta** (05.08) — prestao drugi pisac u sy15; pages.dev hosting se gasi, baza ne

**Preduslovi (bez njih nijedan domen ne može do kraja):**

- [ ] **Korak 0 — mapa identiteta** sy15 `auth.users.id` ↔ 3.0 `users.id` (67 FK kolona) — blokira SVE ispod, a posebno gašenje GoTrue-a
- [ ] **TALAS-F-6 — popis 14 sy15 edge funkcija** — bez inventara se ne zna šta se gasi; jeftino, a otvara F-2 i F-5

**Domeni (redosled iz PLAN_GASENJA, revizija 2):**

- [ ] **Korak 1 — sastanci** (U TOKU, grana `feat/sy15-seoba-sastanci-pb`; ostaje ~61 DEFINER fn + enqueue)
  - [ ] **TALAS-F-2 / X-EMAIL-AUTOMATIKE-10** — javni RSVP magic-link preseliti u 3.0 **pre** gašenja edge sloja (inače pucaju i stari i novi linkovi iz pozivnica)
  - [ ] **PROIZVODNJA-PRACENJE-01 / B4** — ukloniti 501 (zavisi od preseljenih ključeva akcionog plana)
- [ ] **Korak 2 — održavanje** (41 fn, 34 tabele + 34 trigera, `maint-machine-files` 469 MB)
- [ ] **Blok-B1 — loc-most repoint** (živa sekvenca, čeka prozor) → **preduslov za F5c**
- [ ] **F5c — Lokacije native ingest** (nativan writer u `loc_location_movements`) → **preduslov za F5d**
- [ ] **Korak 3 — reversi + lokacije zajedno** (priprema urađena; ostaje ~5–8 dana za `rev_issue_reversal`/`rev_confirm_return`)
- [ ] **F5d — fizičko gašenje mosta** (unschedule `loc_bigtehn_ingest_5min`, DROP `bigtehn_*` keša, prilagoditi monitor) — **zavisi od F5c i koraka 3**
- [ ] **Korak 4 — kadrovska** (58 fn, plate pod allowlist bravom, `employee-docs` + `attendance_events`)
- [ ] **Korak 4b — projektni biro** (32 fn) — **zavisi od koraka 4** (`pb_current_employee_id()` traži `employees`/`departments`/`job_positions`)

**Pisci (bez njih se baza fizički ne gasi):**

- [ ] **Korak 5a — SCADA:** preusmeriti `servoteh-bridge-scada` relej na 3.0, preseliti 5 `scada_*` tabela, prevesti `energetika.service` na 3.0 Prisma (odluka: koliko istorije od 558 MB seliti)
  - [ ] **TALAS-F-5** — SCADA watchdog + loc-sync health u 3.0 scheduler (inače ispad releja niko ne vidi)
  - [ ] **ENERGETIKA-02** — theme-sync bez reload-a (isti fajl, jeftino uz seobu)
- [ ] **Korak 5b — bridge (.mdb masteri)** preusmeriti na 3.0 — **POSLEDNJI PISAC**; BigBit živi do ~01.02.2027, pa kanal mora nastaviti da radi

**Završnica:**

- [ ] **Blok-B3 — konsolidacija dva PG datasource-a** (ukloniti `sy15.prisma`; `reversi.service.ts` + `auth.service.ts`)
- [ ] **Korak 6 — „ostalo"**: `ai_chat_*` (~20 fn), `assessment_*` (~15 fn), `dictation_inbox`, kiosk pomoćne, preostali storage bucketi (`bigtehn-drawings` 995 MB — seliti ili arhivirati)
- [ ] **Blok-B4 — gašenje PostgREST + GoTrue** (poslednje: 9 prijava u 7 dana još ide kroz GoTrue)
- [ ] **Fizičko gašenje sy15** uz `pg_dump` arhivu — moguće tek kad nijedan od tri pisca više ne piše

> **🔴 Tri rečenice koje presuđuju redosled:** (1) `F5d` zavisi od `F5c`, a `F5c` od `Blok-B1` — most
> se ne gasi dok lokacije nemaju nativan upis. (2) **PB zavisi od kadrovske**, a promocija akcione
> tačke (501) od sastanaka. (3) **sy15 se ne može ugasiti dok SCADA relej i .mdb bridge pišu u nju** —
> aplikacija je najmanji pisac (~20 hilj. izmena naspram 4,09 mil.), pa „aplikacija je prešla" i
> „baza se gasi" nisu isti događaj.
...
- **🟨 Verifikacija 06.08 — DELIMIČNO:** sadržaj kartice je 100% prisutan u modalu (`tool-detail-dialog.tsx`) — fali samo forma: samostalna ruta i URL deep-link. Nisko prioritetno.
...
- **✅ Verifikacija 06.08 — URAĐENO:** tesseract.js@5.1.1 je self-hosted (public/tesseract/), `label-ocr.ts` ima localTesseractOptions i isOcrEngineAvailable više ne zavisi od window.Tesseract (commit dee2727b).
...
- **✅ Verifikacija 06.08 — URAĐENO:** `stampa-tab.tsx` runThermalBatch() šalje TSPL2 direktno na TSC preko /locations/labels/print (commit dee2727b).
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** movementLabel() i dalje vraća plain string bez boje; StatusBadge postoji samo za tip lokacije, ne za tip pokreta. Effort S, nema tehničke prepreke.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** nema ni implementacije ni formalne odluke o prenosu 3D modela faze — docs/OTVORENI_POSLOVI.md ga ne pominje.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** assemblyDrawingNo je i dalje samo `<input>` u meta dijalogu; DrawingChip infrastruktura postoji, ali je ožičena isključivo za povezane crteže faza.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** pokriveno stavkom `C6` (Gantt PDF legenda) — i dalje otvoreno, bez grananja po phaseType u gantt-pdf.ts.
...
- **✅ Verifikacija 06.08 — URAĐENO:** julski nalaz je prevaziđen — motor sada postoji u 3.0: posao `pb-enqueue` (scheduler/sy15-cron-jobs.ts) + dispatch u `notify-dispatch.service.ts` (909486dd).
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** exportCsv header i dalje ima 16 kolona, bez „Kašnjenje (d)".
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** WINDOWS niz i dalje Mesec/Kvartal/6 meseci — nema dnevnog zoom nivoa ni horizonta od 24 meseca.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** `pracenje.service.ts:690-694` i dalje 501 — akcioni-plan/sastanci čitaju sy15 uuid ključeve; cutover 1.0 (03.08) je preusmerio FE rute ali NIJE migrirao podatke akcionog plana. Zavisi od koraka 1 seobe sy15 (v. §9).
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** `api/pracenje.ts` i dalje POLL_MS=30000 uz komentar „paritet 1.0"; nema WS/SSE mehanizma.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** `detalj-akcije.tsx` i dalje bez CSV/Štampaj/filtera „završeno" — funkcije postoje samo u globalnom akcioni-plan-tab.tsx.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** TemaModal se i dalje otvara samo iz pm-teme-tab.tsx; globalnog FAB-a nema.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** welcome grana (`ai-chat.tsx:405-414`) je i dalje samo statičan tekst, bez suggestion chip-ova.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** subtitle je i dalje fiksan (projectRef · deljena nit / interno), bez naziva engine-a i bez upozorenja „vide svi".
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** `payslip.ts` i dalje ima jmbg param koji pozivaoci (obracun-view.tsx) ne prosleđuju; „Tip rada" je i dalje potpuno odsutan iz fajla.
...
- **🟨 Verifikacija 06.08 — DELIMIČNO:** computeLiveTotals daje delimičan live preview za dirty red, ali pun K3.3 preview PRE klika „Obračunaj iz grida" i dalje ne postoji — ostaje svestan arhitektonski kompromis (BE autoritativan).
...
- **✅ Verifikacija 06.08 — URAĐENO:** `makeup-paidleave-section.tsx` koristi PAID_LEAVE_CATALOG (optgroup fond/van-fonda + maxDays), a BE DTO validira @IsIn(PAID_LEAVE_CODES) — commit de5390a8. (Godišnji fond od 5 dana i dalje se ne računa — v. B3.)
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** `vacation-section.tsx:99-104` kolone i dalje bez razloga odbijanja/odgovora HR-a; rejection_note postoji samo na HR strani.
...
- **🟨 Verifikacija 06.08 — DELIMIČNO:** akrual „preostalo do danas" je urađen (moj-profil.ts vacationRemaining, zahtev 028/26), ali avans-upozorenje i [AVANS] marker u modalu ZAPOSLENOG i dalje fale — postoje samo na HR admin ekranu (saldo-tab.tsx).
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** VacationModal je i dalje bez kalendarskih dana, inline balans-upozorenja, napomena Pravilnika, potvrde pre slanja i uputstva za neplaćeno.
...
- **✅ Verifikacija 06.08 — URAĐENO:** MakeupModal i PaidLeaveModal imaju „Za koga" picker (teamOpts/forEmp) uz istu IDOR branu resolveSubmitTarget — commit de5390a8.
...
- **✅ Verifikacija 06.08 — URAĐENO:** ChangeRequestModal za ODOBREN GO objašnjava posledicu (termin ostaje do HR odobrenja) — zahtev 026/26, commit 48fe0e57, PR #65.
...
- **✅ Verifikacija 06.08 — URAĐENO:** MakeupModal.save() klijentski validira vikend + ≥8h, a tabela prikazuje badge „🏖 Dan odmora" — commit de5390a8 + 2a496583.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** grep canManageDevPlanFor/scopedEmployeeOptions i dalje 0 — postoji samo BE brana pri upisu kampanje, ne FE scoping pickera.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** `work-order-print.service.ts` i dalje zna samo RnPrintVariant `std|bez-barkoda`; WorkOrderOperationImage postoji u schemi ali se ne ugrađuje u PDF.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** `hr-pdf/assessment.ts` i dalje bez radar/addImage/toDataURL; assessments.tsx:403 zove exportAssessmentPdf bez radar podataka.
...
- **✅ Verifikacija 06.08 — URAĐENO (REFUTED):** dokument sam markira REFUTED i verifikacija to potvrđuje — `dokumenti-tab.tsx:468` „Štampa prikaza" → window.print() (RB-26) je direktan ekvivalent 1.0 revZadPrint.
...
- **🟨 Verifikacija 06.08 — DELIMIČNO:** A0 poster i deep-link „#pos-<id>" i dalje NE postoje. Novo od 05.08: `sistematizacija-section.tsx` nudi PRAVNI PDF/DOCX dokument (tekst, ne vizuelni poster) — druga stvar. Ekvivalenti ostaju: tekstualni OrgReport + pojedinačni „Opis pozicije (PDF)".
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** OrgReport (`izvestaji-tab.tsx:347-394`) je i dalje jedini izveštaj bez ijednog dugmeta za izvoz/štampu.
...
- **✅ Verifikacija 06.08 — URAĐENO:** `solar-sigen.js` je dobio stale badge, a `hmi-drift.mjs` čuva usklađenost HMI kopija (commit f8c04f2a).
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** `hmi-host.tsx` i dalje remount-uje iframe na promenu teme (key={screen:theme}); postMessage „scada-theme" se nikad ne šalje iako ga shim sluša. Jeftino je uraditi uz korak 5 seobe sy15 (v. §9).
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** ANALYTICS_PAGE=200 cap i dalje aktivan (`izvestaji-tab.tsx:38`), a oba CSV izvoza rade nad istim capiranim redovima.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** exportCostsCsv i dalje ima granularnost po RADNOM NALOGU — agregatni all-parts endpoint i dalje ne postoji.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** BarList i tabela „Poslednji kvarovi" i dalje renderuju plain `<span>`, bez onClick/href ka kartonu mašine.
...
- **🟨 Verifikacija 06.08 — DELIMIČNO:** automatika JESTE seljena u 3.0 (posao `maint-deadlines` u scheduler-u, širi obuhvat od originalnog zahteva), ali FE dugme „Proveri rokove" i dalje fali — BE ruta i hook useVehicleDeadlineCheck postoje, nijedna komponenta ih ne zove.
...
- **✅ Verifikacija 06.08 — URAĐENO:** resetPassword sada radi dual-write: GoTrue + bcrypt hash u 3.0 `users.passwordHash` (commit 5542c660) — direktan 3.0 login prihvata novu lozinku.
...
- **✅ Verifikacija 06.08 — URAĐENO:** POST /auth/change-password postoji, a `must_change_password` se sprovodi u auth-context.tsx/login/mob-prijava (commit 5542c660).
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** i dalje samo GET holidays u oba kontrolera — nijedan POST/PATCH/DELETE nigde u backend/src.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** backend je i dalje spreman (project_id u insertSy15Role/updateSy15Role), ali `korisnici-tab.tsx` ga uopšte ne konzumira ni prikazuje (grep 0).
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** stat kartice i dalje tvrdo prikazuju top-4 role; tabela i dalje bez kolone „Dodato" i bez avatar inicijala.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** nema DMY maske (installDmy*/dmyMask = 0 pogodaka), a native `<input type="date">` je čak i porastao — 199 pogodaka u 104 fajla (jul: 152 u 81).
...
- **🟨 Verifikacija 06.08 — DELIMIČNO:** normalize() iz lib/fuzzy.ts dobio je JOŠ jedno mesto (kadrovska/prisustvo/live-rows.ts), ali generalizacija i dalje ne postoji — ~60 tabelarnih filtera je i dalje bez folding-a i bez deljenog matchesSearch.
...
- **⬜ Verifikacija 06.08 — NIJE (i dalje otvoreno):** i dalje samo Ctrl+B i Ctrl/Cmd+K — nema jednoslovnih prečica „/", „n", „r" ni overlay spiska.
...
- **🟨 Verifikacija 06.08 — DELIMIČNO:** sticky proxy hscroll traka (paritet 1.0 stickyHscroll.js) i dalje ne postoji; postoji samo delimična sticky zaglavlje/kolone mera u grid-table.tsx, koja je postojala i u julu.
...
- **🟨 Verifikacija 06.08 — DELIMIČNO:** broj window.confirm poziva je opao (80 fajlova naspram 118/182 u julu), ali jedinstvena ui-kit Confirm komponenta i dalje ne postoji — najmanje 5 nezavisnih ad-hoc implementacija.
...
- **✅ Verifikacija 06.08 — URAĐENO:** `backend/src/modules/scheduler/` postoji na origin/main (commit e2813cc0) — koren svih automatika je otključan.
...
- **✅ Verifikacija 06.08 — URAĐENO:** `sy15-cron-jobs.ts:96-100` kadr-hr-reminders + `:137-140` kadr-qbt-cards; outbox preuzima notify-dispatch.service.ts.
...
- **✅ Verifikacija 06.08 — URAĐENO:** `sy15-cron-jobs.ts:103-106` kadr-corrective (dnevni ekvivalent 07:30).
...
- **✅ Verifikacija 06.08 — URAĐENO:** `sy15-cron-jobs.ts:109-112` kadr-onboarding.
...
- **✅ Verifikacija 06.08 — URAĐENO:** `sy15-cron-jobs.ts` kadr-attendance-alerts, kadr-attendance-digest i kadr-weekly-risk.
...
- **✅ Verifikacija 06.08 — URAĐENO:** `sy15-cron-jobs.ts` sast-action-reminders + sast-meeting-reminders (5-min tik, prozor 25–35 min); dispatch f4eaa651/c766f358.
...
- **✅ Verifikacija 06.08 — URAĐENO:** `sy15-cron-jobs.ts:163-168` sast-weekly-auto, lokalno petak 08h preko belgrade-time.ts (sopstveni DST util).
...
- **✅ Verifikacija 06.08 — URAĐENO:** `sy15-cron-jobs.ts:171-175` maint-deadlines (30 dana unapred).
...
- **✅ Verifikacija 06.08 — URAĐENO:** više nije OUT-OF-SCOPE — `notify-dispatch.service.ts` (909486dd) + `sastanci-dispatch.service.ts` (f4eaa651) rade dequeue + mark_sent/failed, paritet sa 1.0 edge workerima.
...
- **🟨 Verifikacija 06.08 — DELIMIČNO:** 3.0 generiše RSVP link (`sastanci-dispatch.service.ts:362-370`), ali javnu no-JWT obradu i dalje NAMERNO radi sy15 edge (komentar :157-163). Svestan sy15-dug — v. **§9** i TALAS-F-2.
...
- **✅ Verifikacija 06.08 — URAĐENO:** nativni ekran ocenjivača je živ — `kadrovska-mutations.service.ts:2612` daje link /profil?ocena=<raterId> (test kadrovska.audit-k3.spec.ts:105); legacy ocena.html se koristi samo uz eksplicitan env.
...
- **✅ Verifikacija 06.08 — URAĐENO:** `notify-dispatch.service.ts` šalje kadr prilog iz sy15 Storage-a i WhatsApp preko Meta Graph v20 (:331-368) — kanal-diverzitet je vraćen u 3.0.
...
- **🟨 Verifikacija 06.08 — DELIMIČNO:** SCADA alarmi su pokriveni (scada/notifier.js + server.js stale/offline detekcija), ali loc-sync health monitor i watchdog nad samim gateway-em i dalje žive na sy15 pg_cron-u — v. **§9** i TALAS-F-5.

## Aneks B — Već popisano u postojećim planovima (žetva docs, 51 stavki)

- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → PL-01: GET /part-locations?q=<širok> vraća 500 (bind-param overflow u list()); svaki čest karakter u pretrazi obara endpoint (VISOKO, BLOCKER) → **Verifikovano 06.08:** `part-locations.service.ts` koristi EXISTS podupite umesto bind-overflow-a (commit f6842c90), uz regresioni test.
- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → PLK-01: GET /locations/definitions-audit 500 (loc_locations_audit/audit_log drift; migracija add_loc_locations_audit.sql ne postoji u repou; servis bez try/catch, FE bez retry:false) (VISOKO, BLOCKER) → **Verifikovano 06.08:** `locations.service.ts:1255` BigInt→Number + try/catch.
- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → L-22: Pregled predmeta picker traži UKUCAN numerički ID umesto search-liste otvorenih predmeta (U TOKU, ⭐ prvi) (SREDNJE) → **Verifikovano 06.08:** `predmet-tab.tsx:295-460` search-lista otvorenih predmeta.
- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → L-23: hero samo „Predmet #id" + HARDKODOVAN badge „U TOKU" (prikazuje ga i za zatvoren predmet) (SREDNJE) → **Verifikovano 06.08:** `predmet-tab.tsx:66-70,667-676` prikazuje stvaran BigTehn status badge.
- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → L-27: Štampa/Export PDF/CSV celog filtriranog spiska (18 kolona) ne postoji — samo per-red TP nalepnica (SREDNJE, GAP) → **Verifikovano 06.08:** `predmet-print-window.ts` štampa/izvozi ceo filtrirani spisak.
- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → L-14: Pregled po lokacijama — na ekranu fale kolone Ukupno/Status/Akcije (podaci su u CSV-u) (SREDNJE) → **Verifikovano 06.08:** `report-tab.tsx:220-250` ima kolone Ukupno/Status/Akcije.
- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → L-16: Pregled po lokacijama — red nema akcije (klik→istorija, ⚙ RN/TP op-modal, TP nalepnica) (SREDNJE, GAP) → **Verifikovano 06.08:** `report-tab.tsx` kolona actions + ItemHistoryDialog na klik reda.
- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → L-06/L-07: baneri „keš zastareo" i „sync worker zdravlje" gejtovani za admina — magacioner/cnc ne vide upozorenje (SREDNJE) → **Verifikovano 06.08:** `locations.controller.ts` GET sync/health više nije gejtovan samo za admina.
- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → L-33: forma KAVEZ + bulk osiromašena vs 1.0 (SREDNJE) → **Verifikovano 06.08:** `cage-form-dialog.tsx` bulk opseg + BulkShelvesDialog.
- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → L-20/L-21: Sync tab — ingest worker panel i outbound events prikazani kao sirov JSON <pre> umesto kartica/tabele (NISKO) → **Verifikovano 06.08:** `sync-tab.tsx` StatCard + tabela umesto sirovog `<pre>` JSON-a.
- 🟨 CUTOVER_AUDIT_lokacije_2026-07-17.md → PL-02: atribucija izvršioca u ledger upisima = radnik sa RN, ne prijavljeni korisnik (nema User↔Worker veze; TODO(auth)) (NISKO) → **(DELIMIČNO) 06.08:** resolveActorWorkerId postoji i koristi se drugde, ali `part-locations.service.ts:331/342/409` i dalje ima TODO(auth) sa RN-radnik fallback-om.
- ✅ CUTOVER_AUDIT_lokacije_2026-07-17.md → PLK-02: POST /locations/sync/run-now bez DTO brane/confirm/lock — slučajan POST okida ingest posao (NISKO) → **Verifikovano 06.08:** SyncRunNowDto @IsIn([true]) na confirm — bez potvrde vraća 400.
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → RC-17/RC-32: POVRAĆAJ REZNOG ALATA ne postoji sa FE strane (nema dugmeta/dijaloga/hooka; BE ruta cutting-return mrtva) — izdavanje jednosmerno (KRITIČNO) → **Verifikovano 06.08:** POST cutting-return + `cutting-return-dialog.tsx` postoje i koriste se.
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → RC-06/RC-09: TIHE LAŽI — kolona „Na stanju" sabira SVE lokacije (prikazuje ukupno seedovano, ne raspoloživo u magacinu); semafor niske zalihe ne okida kad je magacin prazan (KRITIČNO) → **Verifikovano 06.08:** `magacin-tab.tsx:75-108` stockPresentation() razdvaja warehouseQty od ukupnog.
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → PR-01: PATCH /cutting-tools/:id na nepostojeći UUID vraća 500 umesto 404 (Prisma P2025 bez mapiranja) (SREDNJE) → **Verifikovano 06.08:** updateCuttingTool() mapira Prisma P2025 → NotFoundException.
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → PR-02: GET /documents/:id/signature-pdf 422 — objekat fali u sy15 bucketu reversal-pdf (upload lanac potpisnice sumnjiv) (SREDNJE) → **Verifikovano 06.08:** getSignaturePdfUrl() vraća 404 sa jasnom porukom umesto 422.
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → RB-04: kartica alata bez trenutnog zaduženja/lokacije i pola polja (VISOKO) → **Verifikovano 06.08:** findOneTool je obogaćen klasifikacijom i trenutnom lokacijom (test reversi-r3.service.spec.ts:108).
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → RA-30/31/33: magacin (zbirno) bez trake filtera („Sve lokacije" postoji na BE, nije priključeno na FE) (VISOKO) → **Verifikovano 06.08:** `magacin-tab.tsx:536-588` traka filtera „Sve lokacije" + status-boje.
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → RA-39/40: izveštaj potrošnje ne postoji (VISOKO) → **Verifikovano 06.08:** GET reports/consumption + `consumption-report-dialog.tsx`.
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → RA-43: workbench bez „Urgentno" (VISOKO) → **Verifikovano 06.08:** `workbench-tab.tsx:159-225` sekcija „Urgentno".
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → RA-22/35: bulk štampa nalepnica ne postoji (VISOKO) → **Verifikovano 06.08:** `bulk-print-labels-dialog.tsx` (bulk štampa nalepnica).
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → RC-01…: ceo tab „Rezni alat" sa 4 pod-taba ne postoji kao takav (VISOKO) → **Verifikovano 06.08:** `rezni-alat-tab.tsx:111-114` sa 4 pod-taba (Pregled/Katalog/Po mašinama/Po zaposlenima).
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → RC-43/50/51/53–55: bulk import sa sesijama i stornom ne postoji (VISOKO) → **Verifikovano 06.08:** `bulk-import-dialog.tsx` + `import-rollback-dialog.tsx` (storno sesija).
- ✅ CUTOVER_AUDIT_reversi_2026-07-17.md → RB-46/RB-11: modali „Nova jedinica" i „Izmena artikla" ne postoje (VISOKO) → **Verifikovano 06.08:** `tool-create-dialog.tsx` + `tool-edit-dialog.tsx`, BE rute postoje.
- ⚰️ CUTOVER_AUDIT_reversi_2026-07-17.md → ukupni paritet 23/173 desktop funkcija (13%); put do GO = Drop R0 pa R1–R5 (~5,2 MN); katalog reznog prazan (0 redova — čeka domensku validaciju vlasnika) → **(PREVAZIĐENO) 06.08:** statistika 23/173 (13%) od 17.07 je zastarela — skoro sve gore navedene stavke su naknadno zatvorene, pa merenje više ne opisuje stanje modula.
- 🟨 CUTOVER_AUDIT_odrzavanje_2026-07-17.md / CUTOVER_FOLLOWUP_odrzavanje_2026-07-20.md → Live-RLS matrica (#45/#46): row-scope proven-by-construction ali NIJE dokazan živim testom (operator machine-scope, chief-bez-role, magacioner) — čeka odluku o test-profilima na produ (nije blokada) → **(DELIMIČNO) 06.08:** e2e test postoji, ali eksplicitno kaže da NE dokazuje row-scope na živoj bazi — i dalje čeka odluku o test-profilima na produ.
- ⬜ CUTOVER_FOLLOWUP_odrzavanje_2026-07-20.md → Živi smoke #48: deo-na-WO → skida zalihu atomski jedini je kod-verifikovan a ne živ-dokazan (magacin prazan); odluka 20.07 PRESKAČEMO, izvesti tek na zahtev → **(potvrđeno 06.08)** nema novijeg dokumenta ni komita koji potvrđuje pun živi ciklus posle 20.07.
- 🟨 CUTOVER_AUDIT_sastanci_ai_2026-07-17.md → Mobilni paritet /m/sastanci (2.0 samo read+status) — fale kreiranje/uređivanje akcija i tačaka, priprema po osobi, STT, slike, arhiva, pretraga/kalendar; mobilni ostaje 1.0 do dopune → **(DELIMIČNO) 06.08:** /mob/sastanci ima RSVP/status/„moju pripremu", ali kreiranje i izmena akcija i tačaka i dalje traže desktop (svesno, po docstring-u).
- 🟨 CUTOVER_AUDIT_sastanci_ai_2026-07-17.md → S-P1 AI sitnice: dnevni limit hardkodovan 50 (1.0 env), izgubljena distinkcija alat_neuspesan vs alat_nedostupan, /m/ai ne nastavlja poslednju nit <6h (NISKO) → **(DELIMIČNO) 06.08:** budžet po tipu i semantička distinkcija grešaka su urađeni; /mob/ai nastavak poslednje niti <6h i dalje nije.
- ⚰️ CUTOVER_AUDIT_sastanci_ai_2026-07-17.md → S-P1: sastanci_notification_log — razmotriti REVOKE INSERT za čvršću ogradu (RLS snl_insert dozvoljava authenticated+edit rolu) (NISKO) → **(PREVAZIĐENO) 06.08:** pitanje je aktivno razmotreno i svesno odbijeno (MODULE_SPEC_sastanci_ai_30.md §10) — nije previd nego odluka.
- 🟨 RADNI_PLAN_3.0.md → Blok B(1): Loc-most repoint — kod sletio 18.07, živa sekvenca čeka prozor; blokira gašenje QBigTehn; čeka 4 odluke (auth feed cron, backfill A vs start-od-sada B, granularnost signala, potvrda čitača) → **(DELIMIČNO) 06.08:** kod sletanja postoji (`loc-tp-feed.service.ts`), ali živa sekvenca i dalje čeka prozor (RUNBOOK_LOC_MOST_REPOINT.md nepromenjen) — v. **§9**.
- ⬜ RADNI_PLAN_3.0.md → Blok B(3): Konsolidacija dve PG baze — Reversi+auth još čitaju sy15 preko 2. Prisma datasource-a → **(potvrđeno 06.08)** `backend/prisma/sy15.prisma` je i dalje zaseban datasource, a `reversi.service.ts` i `auth.service.ts` direktno čitaju sy15 — v. **§9**.
- 🟨 RADNI_PLAN_3.0.md → Blok B(4): Gašenje PostgREST + GoTrue (sy15-*) + potpuna dekomisija 1.0 Vite fronta (SSO kapija + /m/*) → **(DELIMIČNO) 06.08:** 1.0 FRONT jeste dekomisioniran (PROXY_1_0_AKTIVAN=false, 05.08), ali PostgREST/GoTrue i sy15 baza su i dalje potpuno živi — koristi ih 161 fajl u backend/src (v. **§9**).
- 🟨 RADNI_PLAN_3.0.md → Blok C2: Native core WRITE probe (net-zero) za TP/kiosk/MRP — off-hours na živoj bazi → **(DELIMIČNO) 06.08:** `e2e/tests/netzero/` obrazac postoji za 5 modula, ali ne i za TP/kiosk/MRP specifično.
- 🟨 RADNI_PLAN_3.0.md → Blok C3: part-locations izvršilac fix — ledger upisuje RN-radnika umesto magacionera (part-locations.service.ts:331/342/409; resolveActorWorkerId postoji) → **(DELIMIČNO) 06.08:** isto kao PL-02 — resolveActorWorkerId postoji, ali ga part-locations i dalje ne koristi.
- ⬜ RADNI_PLAN_3.0.md → Blok C4: RBAC definesApproval/Launch gate (work-orders.service.ts:1038/1080/1095) — čeka Nenadovu poslovnu odluku ko sme da odobri/lansira (nije bag, blokira prod ako pogrešno) → **(potvrđeno 06.08)** `work-orders.service.ts:1076,1137` i dalje nose TODO(auth) za definesApproval/definesLaunch — svesno odloženo, čeka poslovnu odluku.
- 🟨 RADNI_PLAN_3.0.md → Blok E (loose ends): GoTrue SITE_URL (reset lozinke) · servosync2 front pao · Lokacije BigTehn search · Štampa ulazna tačka iz Praćenja → **(DELIMIČNO) 06.08:** GoTrue SITE_URL reset i dalje nije adresiran (workaround: admin vidi lozinku); Lokacije OCR je pokriven; ostatak nije izmeren ni potvrđen.
- ✅ PLAN_PRACENJE_PROIZVODNJE_2026-07.md → F3 dopune tabele (docx zahtevi 1–12): stablo + % gotovosti po poziciji/podsklopu/sklopu, boja reda po tipu, freeze levih kolona pri horizontalnom skrolu, ručni unos količina, auto-status „Kompletirano"→maš/površ DA+100%, datum završetka operacije, brisanje kolone „usko grlo", rok izrade→dokument primopredaje, filteri maš/površ/pozicija, PDF na klik → **Verifikovano 06.08:** `predmet-view.tsx` (stablo + % gotovosti, sticky kolone, ručni unos, „Izvezi PDF") i `rn-view.tsx` (rok → primopredaja).
- 🟨 PLAN_PRACENJE_PROIZVODNJE_2026-07.md → F5 gašenje mosta: repoint Plan proizvodnje + Lokacija na glavnu bazu pa gašenje loc-tp-feed, pg_cron ingest-a i bigtehn_* keša u sy15 → **(DELIMIČNO) 06.08:** F5b je urađen, ali F5c i F5d nisu izvršeni — plan ostaje predlog (konsolidovano u **§9**).
- ⚰️ RECEPT_M12_REDIRECT_PRACENJE_1.0.md / PLAN_F5 §3.2 → O8 (HITNO): 1.0 ekran Praćenja (desktop + /m/pracenje) NIJE cutover-ovan — čita sy15 podatke u koje 3.0 od 19.07 više ne piše (prikazuje zamrznuto, unosi u tabele koje niko ne čita); traži iframe-redirect po obrascu Tehnologije → **(PREVAZIĐENO) 06.08:** ceo 1.0 front je ugašen 05.08 (cutover, PR #95) — uskoobiman iframe-redirect je zamenjen potpunim gašenjem.
- ✅ PLAN_F5_GASENJE_MOSTA.md → F5b (HITNO): Plan proizvodnje native repoint — PP i 1.0 Lokacije rade nad podacima ZAMRZNUTIM 14–15.07 (svako kucanje od tada ne postoji u planu mašina); M11 presuđeno: ide pravo na native, ne aktivirati feed → **Verifikovano 06.08:** `plan-proizvodnje-read.service.ts:26-52` čita direktno native tabele, ne bigtehn_* keš.
- ✅ PLAN_F5_GASENJE_MOSTA.md → F5b-1 GAP: 3.0 predmet_aktivacije NEMA kolonu je_projektovanje_montaza (F1 uvoz preneo samo aktivan/prioritete) → dodati kolonu + proširiti import + re-run, da flag bude spreman za B3/Plan montaže → **Verifikovano 06.08:** kolona `je_projektovanje_montaza` postoji i koristi se (schema.prisma:2930).
- ⬜ PLAN_F5_GASENJE_MOSTA.md → F5c: Lokacije — native ingest (čita tech_processes, upisuje loc_location_movements sa holdback 2min + storno skip) + repoint bigtehn čitanja + penzionisanje syncHealth pragova (inače trajni lažni baneri) → **(potvrđeno 06.08)** `locations.service.ts` i dalje SAMO čita loc_location_movements iz sy15 — nativnog writer-a nema (blokira F5d, v. **§9**).
- ⬜ PLAN_F5_GASENJE_MOSTA.md → F5d: fizičko gašenje — ukloniti LocTpFeedService/rute, unschedule loc_bigtehn_ingest_5min, DROP fn/state, RENAME+2ned soak pa DROP 3 feed keša + view lanac; prilagoditi monitor-sy15.sh; posle O8 soak-a DROP sy15 pracenje_* objekata (uz pg_dump) → **(potvrđeno 06.08)** LocTpFeedService je i dalje aktivno registrovan i nema DROP migracija za bigtehn_* keš — zavisi od F5c (v. **§9**).
- ✅ OTVORENA_PITANJA_TALASI_B-F_2026-07-12.md → C1: tim_lider edit Plana montaže je FANTOMSKI u 1.0 (front pušta, RLS ne — izmene žive samo u localStorage) → 2.0 dati PRAVI edit (montaza.edit + proširiti has_edit_role za tim_lider) → **Verifikovano 06.08:** `permissions.ts:214-224` — tim_lider ima PRAVI edit uz DB grant.
- ⬜ OTVORENA_PITANJA_TALASI_B-F_2026-07-12.md → C3: pogon ne može otvoriti PDF crteža (can_read_production_drawings) — v1 strogi paritet; follow-up odluka za cnc_operater (crteži su IP) → **(potvrđeno 06.08)** nema drawings-read permisije za cnc_operater — i dalje neodlučen follow-up.
- ✅ OTVORENA_PITANJA_TALASI_B-F_2026-07-12.md → D3: must_change_password — dodati boolean + force-change ekran u 2.0 auth → **Verifikovano 06.08:** polje mustChangePassword + force-change ekran postoje (isto kao B2, commit 5542c660).
- ✅ OTVORENA_PITANJA_TALASI_B-F_2026-07-12.md → F1: CMMS notif dispatch je MRTAV na produ (RPC-ovi ne postoje, nema schedulera, ~30 queued od aprila) — seliti samo paritet (log+retry+rules); oživljavanje = post-seoba zadatak → **Verifikovano 06.08:** log + retry + rules portovani (`odrzavanje.service.ts:1340/1350/4634`); dispatch je namerno mrtav — tačno kako je F1 tražio.
- ⬜ OTVORENA_PITANJA_TALASI_B-F_2026-07-12.md → B7/E6: orphan trigeri sast_trg_akcija_new/changed (mejl za akcije nikad zakačen) = backlog; scada_notify_prefs UI NE u v1 (backlog) → **(potvrđeno 06.08)** oba su i dalje svesno u backlog-u, nema koda koji ih zatvara.
- ⬜ PLAN_dorade_2026-07-10.md → D8-v2 BACKLOG: email izveštaji škarta (odmah po totalnom škartu na proizvodnja@+uprava@ sa Σ sati po komadu; nedeljni izveštaj ukupan škart+troškovi) — čeka SMTP/mail infrastrukturu i odluku → **(potvrđeno 06.08)** `notifications.module.ts:12` doslovno kaže „BACKLOG (D8-v2) — NIJE deo ovog modula".
- 🟨 PDF_GAP_2026-07-13.md → 6.199 odobrenih crteža bez PDF-a u drawing_pdfs (~2.300 aktuelnih: aktivan RN / u nacrtu / u primopredaji); pdm-bridge ne može unazad — traži ciljni re-export iz PDM Vault-a; odloženo (Nenad 13.07) → **(DELIMIČNO) 06.08:** ručni upload PDF-a po RN-u (work_order_drawing_pdfs, zahtev 038/26) ublažava simptom, ali 6.199 crteža bez PDF-a je i dalje nerešeno u osnovi.

...
| 6 | Zapisnik: tačke CRUD + reorder + rich text + slike (upload/sign/delete) | R1 read IMPLEMENTED · **R2 IMPLEMENTED+TESTED**: aktivnosti CRUD (max+1 rb) + POST /:id/aktivnosti/reorder + slike upload/sign/delete (R2.2; meta-pre-upload). rich text (FE) R3 |
| 7 | Most teme→zapisnik (seed, dedup po tema_id) | **R2 IMPLEMENTED+TESTED** (POST /:id/aktivnosti/seed-from-teme; BE tx dedup po tema_id; e2e edit) |
...
| 10 | PM teme: ceo životni ciklus + hitno/za_razmatranje/admin_rang + reorder po projektu | R1 read IMPLEMENTED · **R2 IMPLEMENTED+TESTED**: POST/PATCH/DELETE /teme, /teme/:id/hitno · /za-razmatranje · /admin-rang · /dodeli, POST /teme/reorder-rang (admin FE-gate; DB=has_edit_role) |
...
| 24 | AI: `/ai/chat` port — 4 engine-a + tool-use petlja (18 alata kroz GUC) + limit 50/dan UTC | **R2.3 IMPLEMENTED+TESTED** (POST /ai/chat: AiProviderService 4 engine-a; tool-loop skeleton — mokovani engine, oba puta; execTool dispatch 18 alata → ai_chat_* kroz withUserRls; limit UTC; upis istorije withUser+auth.uid(); e2e ai.chat + DTO engine 400). Živi engine/scope alata R4 smoke (traži ključeve) |
| 25 | AI: projektne niti (jedna po projektu, bez ličnih alata, ime autora) | **R2.3 IMPLEMENTED** (resolveConversation reuse najstarije po project_ref; toolsForScope isključuje lične alate; prefiks imena autora). TESTED preko chatWithTools+toolsForScope · živi smoke R4 |
| 26 | AI: vision (resize + upload + sign) + auto-naslov niti | **R2.3 IMPLEMENTED** (multipart `image` → parseImage 6MB/mime → ai-chat-images bucket; generateTitle nove lične niti; GET /ai/images/sign RLS-vidljivost). Klijentski resize (FE) R3 · živi upload R4 |
...
| 29 | 🎤 STT + ✨ refine na 2.0 (zapisnik + chat) — po odluci P4 | **R2.4 IMPLEMENTED+TESTED** (media-ai modul: POST /ai/stt Whisper + POST /ai/refine — AiProviderService; profili refine-a; unit + e2e ai.chat). FE dugmad (🎤/✨) R3 · živi Whisper/OpenAI R4 |
| 30 | e2e permission matrica (read/edit/manage/weekly_move/ai_model/ai.chat + row asercije: učesnik-scope, pm_teme vidljivost, ai svoje-niti, zaključan=409) | **R2 PROŠIRENA — TESTED**: e2e `sastanci-ai-permissions.e2e` **125** (rola×endpoint×200/403 za edit/manage/weekly_move/ai_model/ai.chat; storage; AI chat/rezime/media; DTO 400; route-ordering write); unit **372** (`sy15.service` runIdempotentRls redosled+replay+konflikt; `sastanci.mutations` assertAffected/idempotency/storage; `ai-provider` tool-loop mokovani engine; `ai-chat` execTool dispatch + delete scoping; `media-ai`). Row-asercije (učesnik/pm_teme/ai-niti/zaključan=409) → R4 živi smoke |

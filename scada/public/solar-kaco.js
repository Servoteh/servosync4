...
// koliko traje alarm: "19 h 20 min" / "8 min"
function since(ts) {
  if (!ts) return '';
  const m = Math.max(0, Math.round((Date.now() - Number(ts)) / 60000));
  return m >= 60 ? `${Math.floor(m / 60)} h ${m % 60} min` : `${m} min`;
}
const at = ts => ts ? new Date(Number(ts)).toLocaleString('sr-RS') : '—';
...
async function refresh() {
  ...
  // alarmi sa samog loggera su autoritativni (imaju kod, poruku i tačan početak);
  // ako lista ne stigne, padni na izvedeno stanje (offline + pregrevanje)
  const alarmsOn = ((d && d.alarms) || []).filter(a => a.active);
  const hotExtra = list.filter(x => x.temp != null && x.temp > HOT && !x.alarm).length;
  const alarmN = online ? (alarmsOn.length ? alarmsOn.length + hotExtra : offCount + hotCount) : 0;

  // ---- baner: kvar se mora videti bez skrolovanja ----
  const banner = document.getElementById('albanner');
  if (banner) {
    if (!online) {
      banner.className = 'al-banner on';
      banner.innerHTML = `<b>⛔ blue'Log nedostupan</b><span>${(d && d.error) || 'nema odgovora sa loggera'} — prikazani podaci nisu sveži.</span>`;
    } else if (alarmsOn.length) {
      const prvi = alarmsOn[alarmsOn.length - 1];   // najstariji aktivan = kad je počelo
      const koji = alarmsOn.filter(a => a.address != null).map(a => a.address).sort((x, y) => x - y);
      banner.className = 'al-banner on';
      banner.innerHTML = `<b>🚨 ${alarmsOn.length} aktivnih alarma</b>`
        + `<span>${prvi.message || prvi.code}${koji.length ? ` · adrese ${koji.join(', ')}` : ''}`
        + `${prvi.port ? ` · ${prvi.port}` : ''} · traje ${since(prvi.start)} (od ${at(prvi.start)})</span>`;
    } else {
      banner.className = 'al-banner';
      banner.innerHTML = '';
    }
  }
  ...
  cardsEl.innerHTML = list.length ? list.map(inv => {
    ...
    return `<a class="card ${st}">
      ...
      ${inv.alarm ? `<div class="crow"><span class="k">Alarm</span><span class="vv s-alarm">${inv.alarm.message || inv.alarm.code} · ${since(inv.alarm.start)}</span></div>` : ''}
    </a>`;
  ...
  document.getElementById('summary').innerHTML = `
    ...
    <div class="crow2"><span>Regulacija aktivne snage</span><span class="right"><b class="${plant.activePowerControl ? 's-ok' : ''}">${plant.activePowerControl == null ? '—' : (plant.activePowerControl ? 'uključena' : 'isključena')}</b></span></div>
    <div class="crow2"><span>Alarm na loggeru</span><span class="right"><b class="${plant.loggerAlarm ? 's-alarm' : 's-ok'}">${plant.loggerAlarm == null ? '—' : (plant.loggerAlarm ? 'DA' : 'ne')}</b></span></div>
    <div class="crow2"><span>Podaci osveženi</span><span class="right">${online ? tsTxt : (d && d.error ? d.error : '—')}</span></div>`;

  // ---- desni: alarmi (sa loggera + izvedeno) ----
  const rows = [];
  if (!online) rows.push(['p3', "blue'Log offline — prekid komunikacije" + (d && d.error ? ' (' + d.error + ')' : ''), null]);
  else {
    // 1) pravi alarmi sa loggera — poruka, uređaj, port i tačan početak
    alarmsOn.forEach(a => rows.push(['p3',
      `${a.address != null ? 'INV ' + a.address : (a.deviceName || a.deviceId || 'uređaj')} — ${a.message || a.code}`
      + `${a.port ? ' [' + a.port + ']' : ''} · traje ${since(a.start)}`, a.start]));
    // 2) izvedeno: offline bez alarma na loggeru (npr. lista alarma nije stigla) + pregrevanje
    list.forEach(inv => { if (!inv.online && !inv.alarm) rows.push(['p3', invLabel(inv) + ' — ne javlja podatke', inv.ts]); });
    list.forEach(inv => { if (inv.temp != null && inv.temp > HOT) rows.push(['p2', invLabel(inv) + ' — pregrevanje ' + f1(inv.temp) + ' °C', null]); });
    if (meter && !meter.online) rows.push(['p3', 'Brojilo (Janitza) — nema podataka', null]);
  }
  document.getElementById('alarms').innerHTML = rows.length
    ? rows.map(r => `<div class="alrow"><span class="prio ${r[0]}"></span><span>${r[1]}</span><span class="at">${r[2] ? at(r[2]) : new Date().toLocaleTimeString('sr-RS')}</span></div>`).join('')

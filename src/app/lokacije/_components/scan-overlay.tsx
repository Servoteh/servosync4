...
// Nativni BarcodeDetector (Chrome/Edge/Android WebView). HID čitač i „Unesi ručno"
// su fallback svuda gde detektora nema (Firefox/Safari desktop). Bez eksternih zavisnosti.
...
/**
 * Punoekranski skener barkoda za Lokacije — paritet 1.0 scanModal (kamera →
 * BE razrešavanje → onResult). `accept` filtrira dozvoljene tipove; dvokoračni
 * tok stavka→destinacija bira se tako što roditelj otvara skener dvaput:
 *   1) accept=['ITEM'] za stavku (RNZ/short/compact),
 *   2) accept=['SHELF'] za odredište (LP:/„HALA - POLICA"/šifra police).
 * SHELF poruka pariteta (nejednoznačna polica) prikazuje se kao greška, ne rezultat.
 */
    ...
    async (raw: string) => {
      ...
      try {
        ...
        if (data.kind === 'SHELF' && !data.record)
          return say(data.message ?? `Polica ${code} nije jednoznačno razrešena`, 'error');

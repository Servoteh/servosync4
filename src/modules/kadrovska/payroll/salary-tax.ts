/**
 * salary-tax.ts — 3.0 BE port `src/lib/salaryTax.js` (TALAS G, G3, doktrina §C).
 *
 * PORT 1:1 iz 1.0 (NE „modernizovati" — finansijska logika mora ostati identična;
 * zlatni testovi u `salary-tax.spec.ts` fiksiraju poznati ulaz→izlaz iz 1.0).
 * Obračun zarade po propisima RS — preračun NETO ↔ BRUTO (BRUTO I).
 *
 ...
 *
 * Parametri se menjaju svake godine → drže se odvojeno (PARAMS_BY_YEAR).
 *
 * ⚠️ PORESKE TABLICE U KODU (1.0 paritet, izmereno 13.07): SAMO 2025 i 2026.
 *    Ranije godine NISU u 1.0 kodu — `paramsForYear` pada na DEFAULT_PARAMS (2026),
 *    identično 1.0. Nove godine dodaju se u OVU mapu (i u 1.0), ne u formulu.
 */

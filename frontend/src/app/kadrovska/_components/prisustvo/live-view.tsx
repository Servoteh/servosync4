...
import { useAttendanceEvents, type AttendanceEventRow } from '@/api/kadrovska';
...
export function LiveView() {
  // Spoj imenik ⨝ v_attendance_now živi u ./live-rows (deli ga i /mob/prisustvo).
  const live = useLiveAttendance(true);
  ...
  const filtered = useMemo(
    () => filterLiveRows(rows, statusFilter, query),
    [rows, query, statusFilter],
  );

  const chips: { label: string; value: number; tone?: 'default' | 'warn' | 'danger' | 'accent' }[] = [
    { label: 'Prisutno', value: live.counts.prisutan, tone: 'accent' },
    { label: 'Na pauzi', value: live.counts.pauza, tone: 'warn' },
    { label: 'Odsutno', value: live.counts.odsutan },
  ];
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <Button variant="ghost" onClick={reload} title="Ponovo učitaj iz baze" loading={live.isFetching}>

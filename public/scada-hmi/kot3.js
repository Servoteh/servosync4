...
document.addEventListener('click', e => {
  ...
  if(el.dataset.rt){
    ...
    window.__scadaConfirm(`${rc.room}: ${mode === 'cool' ? 'hlađenje' : 'grejanje'} CILJ → ${nv}°C?`).then(ok => { if(ok) lxRoomTemp(rc.key, mode, nv); });
  } else if(el.dataset.lxfan){
    const t = byKey[el.dataset.lxfan], n = Number(el.dataset.n);
    window.__scadaConfirm(`${t.room || t.name}: ventilator brzina ${n}?`).then(ok => { if(ok) lxWrite(el.dataset.lxfan, n); });
  } else if(el.dataset.lxtgl){
    const t = byKey[el.dataset.lxtgl], on = el.classList.contains('on');
    window.__scadaConfirm(`${t.room || 'Klima'}: klimatizacija ${on ? 'ISKLJUČITI' : 'UKLJUČITI'}?`).then(ok => { if(ok) lxWrite(el.dataset.lxtgl, on ? 0 : 1); });
  } else if(el.dataset.lxsw){                          // postrojenje/dvorište switch On/Off
    const t = byKey[el.dataset.lxsw], on = el.classList.contains('on');
    window.__scadaConfirm(`${t.name}: ${on ? 'ISKLJUČITI' : 'UKLJUČITI'}?`).then(ok => { if(ok) lxWrite(el.dataset.lxsw, on ? 0 : 1); });
  } else if(el.dataset.lxstep){                        // mali ValueSelector (dugmad)
    const t = byKey[el.dataset.lxstep], n = Number(el.dataset.n);
    window.__scadaConfirm(`${t.name}: postaviti ${n}?`).then(ok => { if(ok) lxWrite(el.dataset.lxstep, n); });
    ...
    window.__scadaConfirm(`Postaviti ${t.name} = ${v}?`).then(ok => { if(ok) lxWrite(el.dataset.lxset, v); });
  }
});
...
// ===== Trend (24h) — hmiChart + /api/loxone/history (pattern kao solar-kaco skDraw/skLegend) =====
// Serije se izvode DINAMIČKI iz ključeva prvog uzorka: "analog:*", "mix:*", "rooms_avg".
const K3_COLORS = ['#E8523A','#4C8DFF','#27AE60','#C9A91A','#9B59B6','#1ABC9C'];
let K3_SERIES = [];
let K3_EN = (() => { try { return JSON.parse(localStorage.getItem('k3Series')) || {}; } catch(_) { return {}; } })();
let K3_SRV = [];
let _k3Keys = '';
function k3Label(k){
  if(k === 'rooms_avg') return 'Prosek soba';
  const name = k.slice(k.indexOf(':') + 1);
  if(k.indexOf('analog:') === 0 && /^Temperatura\s+/i.test(name)) return name.replace(/^Temperatura\s+/i, '');
  return name;
}
function k3BuildSeries(){
  if(!K3_SRV.length) return;
  const keys = Object.keys(K3_SRV[0]).filter(k => k !== 't');
  const sig = keys.join(',');
  if(sig === _k3Keys) return;
  _k3Keys = sig;
  K3_SERIES = keys.map((k, i) => ({ k, label: k3Label(k), color: K3_COLORS[i % K3_COLORS.length] }));
  K3_SERIES.forEach(s => { if (K3_EN[s.k] === undefined) K3_EN[s.k] = true; });
  k3Legend();
}
async function k3FetchSrv(){ try { const j = await (await fetch('/api/loxone/history')).json(); K3_SRV = j.samples || []; k3BuildSeries(); k3Draw(); } catch(_){} }
function k3Legend(){
  const h = document.getElementById('k3Legend'); if (!h) return;
  h.innerHTML = K3_SERIES.map(s => `<span class="sg-chip ${K3_EN[s.k] ? '' : 'off'}" data-k3series="${s.k}"><span class="dot" style="background:${s.color}"></span>${s.label}</span>`).join('');
}
function k3Draw(){
  const drawn = hmiChart(document.getElementById('k3Chart'), K3_SRV, K3_SERIES, K3_EN);
  const empty = document.getElementById('k3Empty');
  if (empty){ empty.style.display = drawn ? 'none' : 'flex';
    if (!drawn) empty.textContent = K3_SRV.length < 2 ? 'Prikupljam podatke — grafik se puni tokom dana (uzorak ~1 min).' : 'Uključi bar jednu seriju.'; }
}
document.addEventListener('click', e => {
  const c = e.target.closest('[data-k3series]'); if (!c) return;
  const k = c.dataset.k3series; K3_EN[k] = !K3_EN[k];
  try { localStorage.setItem('k3Series', JSON.stringify(K3_EN)); } catch(_){}
  k3Legend(); k3Draw();
});
window.addEventListener('resize', () => k3Draw());

function clock(){ document.getElementById('clock').textContent = new Date().toLocaleString('sr-RS'); }
buildSyn(); k3Legend(); clock(); setInterval(clock, 1000);
load(); setInterval(load, 2000);
k3FetchSrv(); setInterval(k3FetchSrv, 60000);

...
document.addEventListener('click', e => {
  ...
  const c = (msg, fn) => { window.__scadaConfirm(msg).then(ok => { if(ok) fn(); }); };
...
async function refresh() {
  ...
  document.getElementById('cmdbar').innerHTML = !online
    ? `<span style="color:var(--muted);font-size:12px">Komande nedostupne — kontroler nije na mreži (čeka servis na licu mesta)</span>`
